...
- The default branch is protected. Create a topic branch, open a pull request, resolve every conversation, and merge only after the required `architecture-quality`, `dotnet-tests`, and `playwright` checks pass.
...
- Default branch đã được bảo vệ. Hãy tạo topic branch, mở pull request, giải quyết toàn bộ conversation và chỉ merge khi các check bắt buộc `architecture-quality`, `dotnet-tests` và `playwright` đều pass.
